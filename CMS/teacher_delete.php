...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($id > 0) {
        if ($action === 'delete') {
            // 1. Deactivate User Account (so they can't login)
            $deact_user = $conn->prepare("UPDATE users SET is_active = 0 WHERE related_id = ? AND role = 'teacher'");
            if ($deact_user) { $deact_user->bind_param("i", $id); $deact_user->execute(); $deact_user->close(); }

            // 2. Unassign Class Master role
            ...
            // 3. Remove Teacher Assignments (Unlink)
            ...
            // 4. Remove Teacher Subjects (Unlink)
            ...
            $react_user = $conn->prepare("UPDATE users SET is_active = 1 WHERE related_id = ? AND role = 'teacher'");
            if ($react_user) { $react_user->bind_param("i", $id); $react_user->execute(); $react_user->close(); }
            ...
            // 5. Remove Salary Payments
            $check_sp = $conn->query("SHOW TABLES LIKE 'salary_payments'");
            if ($check_sp && $check_sp->num_rows > 0) {
                $del_sp = $conn->prepare("DELETE FROM salary_payments WHERE teacher_id = ?");
                if ($del_sp) { $del_sp->bind_param("i", $id); $del_sp->execute(); $del_sp->close(); }
            }

            // 6. Remove Teacher Attendance
            $check_ta = $conn->query("SHOW TABLES LIKE 'teacher_attendance'");
            if ($check_ta && $check_ta->num_rows > 0) {
                $del_ta = $conn->prepare("DELETE FROM teacher_attendance WHERE teacher_id = ?");
                if ($del_ta) { $del_ta->bind_param("i", $id); $del_ta->execute(); $del_ta->close(); }
            }
